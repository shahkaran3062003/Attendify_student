        ...
        butLeft.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(rightButtonListener != null ) {
                    leftButtonListener.onNavigationButtonClicked(PREVIOUS, previousMonth);
//                    mapDateToDesc = arr[0];
//                    mapDateToTag = arr[1];
                }
                else {
        ...
        butRight.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(leftButtonListener != null) {
                    leftButtonListener.onNavigationButtonClicked(NEXT, nextMonth);
//                    mapDateToDesc = arr[0];
//                    mapDateToTag = arr[1];
                }
                else {
    ...
    private View readyButton(final int date) {
        ...
        if(mapDescToProp != null) {
            ...
            if(mapDateToDesc != null && (mapDateToDesc.get(date) != null) && !mapDateToDesc.get(date).equals("default")) {

                prop = mapDescToProp.get(mapDateToDesc.get(date));

            }
            if(prop != null && prop.layoutResource!=-1) {
                btn=LayoutInflater.from(context).inflate(prop.layoutResource,null);

                if(prop.dateTextViewResource!=-1){ ((TextView)btn.findViewById(prop.dateTextViewResource)).setText(Integer.toString(date));}
                btn.setEnabled(prop.enable);
            }
            else {
                btn = new Button(context);
                ((Button)btn).setText(Integer.toString(date));
            }
        }
        else {
            btn = new Button(context);
        ...
        if(mapDateToTag != null) btn.setTag(mapDateToTag.get(date));
        btn.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                selectedDate.set(Calendar.DAY_OF_MONTH, date);
                if(listener != null) {
                    listener.onDateSelected(btn, selectedDate, mapDateToDesc.get(date));
                }
